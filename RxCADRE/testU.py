...
plt.title('MODELLED vs OBSERVED HORIZONTAL WINDSPEEDS ')
plt.plot(subset_time[-rx.run_min:],np.array(uWRF)[:,0],'r:' ,label='WRF-SFIRE (8 m)')
# plt.plot(subset_time[-rx.run_min:],np.array(uCSU)[-rx.run_min:,0],'k:', label='CSU winds 6.2m')
# plt.plot(subset_time[-rx.run_min:],np.array(uCSU)[-rx.run_min:,1], 'k-.', label='CSU winds 11.3m')
meanCSU8 = (np.array(uCSU)[-rx.run_min:,0] + np.array(uCSU)[-rx.run_min:,1]) /2.
plt.plot(subset_time[-rx.run_min:],meanCSU8,'k:', label='observed (interpolated to 8m)')
plt.ylim([0,5.5])
ax = plt.gca()
...
plt.plot(subset_time[-rx.run_min:],np.array(uWRF)[:,1], 'r', label='WRF-SFIRE (25 m)')
meanCSU25 = (np.array(uCSU)[-rx.run_min:,2] + np.array(uCSU)[-rx.run_min:,3]) /2.
plt.plot(subset_time[-rx.run_min:],meanCSU25,'k', label='observed (interpolated to 25 m)')
# plt.plot(subset_time[-rx.run_min:],np.array(uCSU)[-rx.run_min:,2], 'k:',label='CSU winds 21.1m')
# plt.plot(subset_time[-rx.run_min:],np.array(uCSU)[-rx.run_min:,3],'k-.' ,label='CSU winds 30.7m')
plt.tight_layout()
plt.legend()
plt.savefig(rx.fig_dir + 'HorizontalWind.pdf')
plt.show()
plt.close()
