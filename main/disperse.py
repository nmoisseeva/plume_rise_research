...
for nCase,Case in enumerate(RunList):
    ...
    if zCL < zi + zstep:
        ...
        plt.gca().set(ylabel=r'normalized concentration',xlabel='cross-wind distance [m]')
        plt.tight_layout()
        # plt.show()
...
plt.subplot(3,1,2)
plt.title(r'(b) FIRELINE LENGTH EFFECT')
for nCase,Case in enumerate(RunList):
    print('...case: %s' %Case)
    #create an interpolated profile of temperature
    ...
    sigmaG = (fitWf[0]*wf[runNumber] + fitWf[1])/2
...
#loop through all LES cases
plt.subplot(3,1,3)
plt.title(r'(c) AMBIENT WIND EFFECT')
for nCase,Case in enumerate(RunList):
    print('...case: %s' %Case)
    #create an interpolated profile of temperature
    if Case[-1:]=='T' or Case[-1:]=='E':
...
#length tests
RunList =   ['W5F8R2','W5F6R2']
RunLbl =   ['low intensity','high intensity']
runCnt = len(RunList)                           #count number of cases

#loop through all LES cases
plt.subplot(3,1,1)
plt.title(r'(a) FIRELINE HEAT EFFECT')
for nCase,Case in enumerate(RunList):
    print('...case: %s' %Case)
    #create an interpolated profile of temperature
    if Case[-1:]=='T' or Case[-1:]=='E':
