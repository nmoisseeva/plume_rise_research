...
#plot contours of cross-fireline velocity (V)
fig = plt.figure(figsize=(6,6))
ax = plt.gca()
# ---v contours and colorbar
im = ax.imshow(v_tave, origin='lower', cmap=cmocean.cm.tempo)
cbari =fig.colorbar(im, orientation='horizontal', fraction=0.046, pad=0.1)
cbari.set_label('horizontal velocity v $[m s^{-2}]$')
# cntrf = ax.contourf(v_tave, cmap=plt.cm.PuOr_r, levels=np.arange(-7,7.1,0.5),extend='both')
# cbarf = fig.colorbar(cntrf, orientation='horizontal',fraction=0.046, pad=0.1)
# cbarf.set_label('v $[m s^{-2}]$')
ax.set_xlabel('x grid (#)')
ax.set_ylabel('height AGL [m]')
ax.set_yticks(np.arange(0,len(lvl),10))
ax.set_yticklabels(lvl[::10])
...
axh.tick_params(axis='y', colors='red')
ln = axh.plot(ghfx_tave, 'r-')
plt.savefig(fig_dir + 'TimeAverage/AVE_hfx_v-_temp_%s.pdf' %tag)
plt.show()
print('.....-->saved in: %s' %(fig_dir + 'TimeAverage/AVE_hfx_v-_%s.pdf' %tag))

#plot contours of along-fireline velocity (U)
fig = plt.figure(figsize=(6,6))
ax = plt.gca()
...
axh.tick_params(axis='y', colors='red')
ln = axh.plot(ghfx_tave, 'r-')
plt.savefig(fig_dir + 'TimeAverage/AVE_hfx_u_temp_%s.pdf' %tag)
plt.show()
print('.....-->saved in: %s' %(fig_dir + 'TimeAverage/AVE_hfx_u_%s.pdf' %tag))
